...
static inline void emit(const u32 insn, struct otx2_bpf_arch_ctx *ctx)
{
...
static inline void emit_a64_mov_i(const int is64, const int reg,
				  const s32 val, struct otx2_bpf_arch_ctx *ctx)
{
	u16 hi = val >> 16;
	u16 lo = val & 0xffff;
...
static inline int bpf2a64_offset(int bpf_insn, int off,
				 const struct otx2_bpf_arch_ctx *ctx)
{
...
static inline int epilogue_offset(struct otx2_bpf_prog *p, const struct otx2_bpf_arch_ctx *ctx)
{
	int to = p->epilogue.offset;
	int from = ctx->offset;
...
static int emit_bpf_tail_call(struct otx2_bpf_arch_ctx *ctx)
{
...
//		int ret;
		...
		if (oinsn->to_jmp) {
			pr_err("oinsn->to_jmp exist for oinsn->idx=%d\n", oinsn->idx);
			func_addr = load_addr + (oinsn->to_jmp->ctx.offset  * 4);
		} else {
			func_addr = insn->imm;
		}
#endif
	...
	case BPF_JMP | BPF_EXIT:
		...
		jmp_offset = epilogue_offset(p, ctx);
		pr_err("jmp offset for i=%d offset=%d\n",
		       i, jmp_offset);
		check_imm26(jmp_offset);
		emit(A64_B(jmp_offset), ctx);
#endif
...
static int build_body(struct otx2_bpf_arch_ctx *ctx, bool extra_pass)
{
...
static int validate_code(struct otx2_bpf_arch_ctx *ctx)
{
	int i;
